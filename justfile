...
install-ourchester:
   mkdir -p "{{ LAUNCH_AGENTS }}"
   cp com.ourchester.upgrade.plist "{{ LAUNCH_AGENTS }}/"
   launchctl load "{{ LAUNCH_AGENTS }}/com.ourchester.upgrade.plist"
   @echo "ourchester upgrade job installed"

install-all: install-github install-terraform install-uv install-ourchester
...
uninstall-ourchester:
   launchctl unload "{{ LAUNCH_AGENTS }}/com.ourchester.upgrade.plist"
   rm "{{ LAUNCH_AGENTS }}/com.ourchester.upgrade.plist"
   @echo "ourchester upgrade job uninstalled"

uninstall-all: uninstall-github uninstall-terraform uninstall-uv uninstall-ourchester
...
list-loaded:
   ...
   @echo "\nourchester upgrade job:"
   @launchctl list | grep com.ourchester.upgrade || echo "Not loaded"
...
show-logs:
   ...
   @echo "\nourchester upgrade logs:"
   @cat /tmp/com.ourchester.upgrade.out /tmp/com.ourchester.upgrade.err 2>/dev/null || echo "No logs found"
...
format-plists:
   ...
   plutil -convert xml1 com.ourchester.upgrade.plist
